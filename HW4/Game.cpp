...
GameStatus Game::addPlayer(const char* player_name,const char* weapon_name,Target target1,int hit_strength) {
    int i = 0;
    for (; i < players_num; i++) {
...
GameStatus Game::nextLevel(const char* player_name){
...
GameStatus Game::makeStep(const char* player_name){
    for(int i=0;i<players_num;i++){
        if(players[i]->isPlayer(player_name)){
            players[i]->makeStep();
            return SUCCESS;
...
GameStatus Game::addLife(const char* player_name){
    for(int i=0;i<players_num;i++){
        if(players[i]->isPlayer(player_name)){
            players[i]->addLife();
            return SUCCESS;
...
GameStatus Game::addStrength(const char* player_name, int strength_to_add){
...
GameStatus Game::fight(const char* player_name1, const char* player_name2){
